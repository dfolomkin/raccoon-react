...
import { FETCH_TYPE, ROUTES, UPLOADS_BASE_URL } from 'shared/constants'
...
export const ArticleEdit: React.FC<{ articleStore: ArticleStore }> = observer(
  ({ articleStore }) => {
    ...
    useEffect(() => {
      if (id !== undefined && data) {
        setArticleData(data as ArticleForm)
      }
    }, [data, id])
    ...
    }, [data, fetchType, navigate, resetArticle])
    ...
    const handleFormChange = (event: React.ChangeEvent<HTMLFormElement>) => {
      if (event.target.id === FORM_FIELDS.imageFile) {
        ...
        const form: HTMLFormElement & { elements: FormFields } =
          event.target.form
        const {
    ...
    const handleFormSubmit = async (
      ...
      if (id !== undefined) {
        updateArticle(Number(id), formData)
      } else {
